...
public class BacktestEnginePerformanceResult
{
    ...
    internal static BacktestEnginePerformanceResult FromStates(string name, BacktestingOptions options, IEnumerable<BacktestingCandleResult> states, Stopwatch sw)
    {
        return new()
        {
    ...
    public class IndicatorsPerformanceResult(IEnumerable<BacktestingCandleResult> states)
    {
        ...
        { 
            get
            {
                ...
                var winRate = (double)closedPositions.Count(cp => cp.GetPNL() > 0d) / closedPositions.Count();
                return Math.Round(winRate * 100, 2);
            }
        }

        public double AvgWin => Math.Round(states.SelectMany(state => state.ClosedPositions).Where(cp => cp.GetPNL() > 0).Average(cp => cp.GetPNL()), 2);
        public double AvgLoss => Math.Round(states.SelectMany(state => state.ClosedPositions).Where(cp => cp.GetPNL() <= 0).Average(cp => cp.GetPNL()), 2);
    ...
    public class BacktestPerformanceResult(string name, BacktestingOptions options, IEnumerable<BacktestingCandleResult> states, Stopwatch sw)
    {
